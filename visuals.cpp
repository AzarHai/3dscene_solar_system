...
void SpecialKey(int key, int x, int y)
{
    ...
    {
    case GLUT_KEY_UP:
        if (animate) worldX -= 1.0f;
        break;
    case GLUT_KEY_DOWN:
        if (animate) worldX += 1.0f;
        break;
    case GLUT_KEY_LEFT:
        if (animate) worldY -= 1.0f;
        break;
    case GLUT_KEY_RIGHT:
        if (animate) worldY += 1.0f;
        break;
...
void MouseWheel(int button, int state, int x, int y) {
    if (animate && state == GLUT_DOWN) {
        if (button == GLUT_LEFT_BUTTON) {
            // Левый клик для анимации или другого действия
        } else if (button == 3) { // Вращение вверх (увеличение масштаба)
            scaleFactor += 0.1f;
        } else if (button == 4) { // Вращение вниз (уменьшение масштаба)
            if (scaleFactor > 0.1f) {
                scaleFactor -= 0.1f;
            }
        }
